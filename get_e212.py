...
itu_report_complementary = ('http://www.itu.int/dms_pub/itu-t/opb/sp/'
                            'T-SP-OB.1117-2017-OAS-ZIP-E.zip')
...
transform = {
    ...
    'French Guiana (French Department of)': 'French Guiana',
...
for i, row in enumerate(table.rows[1:]):
    ...
    if mccmnc:
        ...
        try:
            ...
            print('{} ({}): {} - {}'.format(countryname, country.alpha_2,
                                            mccmnc, network))
            networkdict[mcc][mnc] = (country.alpha_2, network)
        except ValueError:
            pass
...
r = requests.get(itu_report_complementary)
r.raise_for_status()

with ZipFile(BytesIO(r.content), 'r') as f:
    with f.open('Anx-E.212MCC-2017-E.docx') as bytedoc:
        doc = docx.Document(BytesIO(bytedoc.read()))
        table = doc.tables[1]
        for i, row in enumerate(table.rows[2:]):
            countryname = row.cells[2].paragraphs[0].text
            mcc = row.cells[0].paragraphs[0].text
            if mcc not in ['0XX', '1XX', '8XX', '901']:
                try:
                    mcc = int(mcc)
                    networks[mcc]
                except KeyError:
                    if countryname in ['Kosovo*']:
                        networks[mcc] = 'XK'
                        continue
                    if countryname in transform:
                        countryname = transform[countryname]
                    networks[mcc] = countries.get(name=countryname).alpha_2
